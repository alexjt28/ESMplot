...
from climate_analysis.climatology import clmMonTSLL
...
def seasavg_PminE_LL(months: list,
  ...
  Emult: :class:'float', Optional - default = 86400. which converts m/s to mm/day
         A float value that becomes a scalar to evaporation values. Assumes original variable is in m/s.
...
def seasavg_rainiso_LL(iso_type:str,
  ...
  Required parameters:
  ----------------------------
  iso_type,months,path,begi,endi
...
def seasavg_vaporiso_LL(iso_type: str,
  ...
  Required parameters:
  ----------------------------
  iso_type,months,path,begi,endi,level
  ...
  wtvar_name: :class:'string' - Optional, default = 'Q'
              Variable to weight the isotopic values by. Default is 'Q', which for CESM is specific 
              humidity. 'Q' is the typical variable by which to weight vapor isotopes. 

...
def seascyc_IVT_vec_TLL(path: str,
  ...
  u_lev = xr.DataArray(interp_hybrid_to_pressure(data=u_data,ps=psrf,hyam=hyam,hybm=hybm,p0=p0Pa,new_levels=plev*100.),
                       dims=['time','lev','lat','lon'],coords=dict(time=data.time,lev=plev,lat=data.lat,lon=data.lon))
  ...
  # Calculate dp with function from geocat.f2py
  dp = dpres_plevel(plev,pbot,ptop)  # need to figure out the issue with this function from geocat
  print(dp)
  ...
  UQ_time = xr.DataArray(UQ_wgt,dims=['time','lev','lat','lon'],coords=dict(time=np.arange(12),lat=data.lat,lon=data.lon),
                         ).sum(dim='lev')
  VQ_time = xr.DataArray(VQ_wgt,dims=['time','lev','lat','lon'],coords=dict(time=np.arange(12),lat=data.lat,lon=data.lon),
                         ).sum(dim='lev')
  print(UQ_time)

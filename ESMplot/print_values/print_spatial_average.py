...
def print_global_average(case: str,
    ...
    ----------------------
    case 
     class: 'string', Case name or variable name of the global variable.

    variable 
     class: 'xarray.DataArray', Global variable from which to calculate global average. 
                                NOTE: Must be of size [lat x lon] and dims=['lat','lon']

    units
     class: 'string', String defining units of the variable.
    '''
...
def print_region_average(case: str,
    ...
    slat, nlat, wlon, elon
     class:'float', Latitude and longitude values at the southern, northern, wester, and eastern
                    boundary of the desired region. Matches to nearest index in variable's 
                    coordinate array. NOTE: For LAT, + value denote °N and - values denote °S.
                    For LON, + values denote °E and - values denote °W.
    '''
    ...
    latarray, lonarray = lat_lon_index_array(lat=variable.lat,lon=variable.lon,
    ...
    print(str(case)+' region avg '+str(lats_val-latadj)+'-'+str(latn_val+latadj)+'('+str(lats)+','+str(latn)+'),'+str(
              lonw_val-lonadj)+'-'+str(lone_val+lonadj)+'('+str(lonw)+','+str(lone)+') ('+str(units)+') = '+str(
              np.array(rwgt.mean(('lon','lat')))))
...
def print_point_average(case: str,
    ...
    latpt, lonpt
     class: 'float', Latitude and longitude values for the desired point. Matches to nearest index of midpoint in 
                     variable's coordinate array. NOTE: For LAT, + value denote °N and - values denote °S.
                     For LON, + values denote °E and - values denote °W.
    ''' 
